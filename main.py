...
def do_stuff():
    ...
    my_mesh = trimesh.load('LabradorLowPoly.stl')
    outline = trimesh.path.polygons.projected(my_mesh, [1, 0, 0],
                                    origin=None, pad=1e-05, tol_dot=0.01, max_regions=200)

    print(type(outline))

    trimesh.path.polygons.plot(outline, show=True)
